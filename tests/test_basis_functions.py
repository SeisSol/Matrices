    ...
    def test_thetha_a_derivative(self):
        for order in range(7):
            for i in range(9):
                ...
                self.assertRelativelyEqual(
                    self.numerical_derivative(f, 0.1 * (i + 1)),
                    basis_functions.diff_theta_a(0.1 * (i + 1), order),
                    atol=1e-4,
                    rtol=1e-4,
                )
        ...
        for order_i in range(7):
            for order_j in range(7):
                for i in range(9):
                    ...
                    self.assertRelativelyEqual(
                        self.numerical_derivative(f, 0.1 * (i + 1)),
                        basis_functions.diff_theta_b(0.1 * (i + 1), order_i, order_j),
                        atol=1e-4,
                        rtol=1e-4,
                    )
        ...
        for order_i in range(7):
            for order_j in range(7):
                for order_k in range(7):
                    for i in range(9):
                        f = lambda xi: basis_functions.theta_c(
                            xi, order_i, order_j, order_k
                        )
                        self.assertRelativelyEqual(
                            self.numerical_derivative(f, 0.1 * (i + 1)),
                            basis_functions.diff_theta_c(
                                0.1 * (i + 1), order_i, order_j, order_k
                            ),
                            atol=1e-4,
                            rtol=1e-4,
                        )
